...
impl RgbColorModelHex {
    ...
    pub fn set_saturation_modulation(&mut self, value:SaturationModulation) -> &mut RgbColorModelHex {
        ...
        for a in e.attributes().with_checks(false) {
            match a {
                Ok(ref attr) if attr.key == b"r" => {
                    &mut self.red.set_value_string(get_attribute_value(attr).unwrap());
                },
                Ok(ref attr) if attr.key == b"g" => {
                    &mut self.green.set_value_string(get_attribute_value(attr).unwrap());
                },
                Ok(ref attr) if attr.key == b"b" => {
                    &mut self.blue.set_value_string(get_attribute_value(attr).unwrap());
                },
                Ok(ref attr) if attr.key == b"val" => {
                    &mut self.val.set_value_string(get_attribute_value(attr).unwrap());
                },
                Ok(_) => {},
                Err(_) => {},
            }
    ...
    pub(crate) fn write_to(&self, writer: &mut Writer<Cursor<Vec<u8>>>) {
        let empty_flag = self.tint.is_none() && self.shade.is_none() && self.saturation_modulation.is_none();
