...
const ENCRYPTION_INFO_PREFIX: [u8; 8] = [0x04, 0x00, 0x04, 0x00, 0x40, 0x00, 0x00, 0x00]; // Version and reserved bytes
...
const BLOCK_KEYS_DATA_INTEGRITY_HMAC_KEY: [u8; 8] =
    [0x5f, 0xb2, 0xad, 0x01, 0x0c, 0xb9, 0xe1, 0xf6];
const BLOCK_KEYS_DATA_INTEGRITY_HMAC_VALUE: [u8; 8] =
    [0xa0, 0x67, 0x7f, 0x02, 0xb2, 0x2c, 0x84, 0x33];
const BLOCK_KEYS_KEY: [u8; 8] = [0x14, 0x6e, 0x0b, 0xe7, 0xab, 0xac, 0xd0, 0xd6];
...
const KEY_SPIN_COUNT: usize = 100_000;
const KEY_HASH_ALGORITHM: &str = "SHA-512";
...
/// # Requirements
///
/// Ensure that the `rand` crate is included in your `Cargo.toml`:
///
/// ```toml
/// [dependencies]
...
pub fn encrypt_sheet_protection(password: &str, sheet_protection: &mut SheetProtection) {
    ...
    let key = convert_password_to_hash(password, &salt, KEY_SPIN_COUNT);
...
/// Encrypts the provided data and writes the encrypted file to the specified
/// filepath. Uses the provided password to encrypt the data.
pub fn encrypt<P: AsRef<Path>>(filepath: &P, data: &[u8], password: &str) {
    generate_random_bytes!(hmac_key, 64);
    generate_random_bytes!(key_salt_value, 16);
    generate_random_bytes!(package_key, 32);
    generate_random_bytes!(package_salt_value, 16);
    ...
    let encrypted_package =
        crypt_package(true, package_block_size, &package_salt_value, &package_key, data);

    // Generate HMAC key and encrypt it
    let hmac_key_iv =
        create_iv(&package_salt_value, package_block_size, &BLOCK_KEYS_DATA_INTEGRITY_HMAC_KEY);
    ...
    let hmac_value_iv =
        create_iv(&package_salt_value, package_block_size, &BLOCK_KEYS_DATA_INTEGRITY_HMAC_VALUE);
    ...
    let encrypted_verifier_hash_input =
        crypt(true, &verifier_hash_input_key, &key_salt_value, &verifier_hash_input).unwrap();
    ...
    let encrypted_verifier_hash_value =
        crypt(true, &verifier_hash_value_key, &key_salt_value, &verifier_hash_value).unwrap();
...
/// Creates a byte buffer containing a 32-bit unsigned integer in little-endian
/// format. If `buffer_size` is specified and greater than 4, the buffer is
...
fn build_encryption_info(
    ...
    [ENCRYPTION_INFO_PREFIX.to_vec(), result].concat()
}
...
mod tests {
    ...
    fn test_hash() {
        let package_salt_value = hex!("4c251b321d85cecfcb6d952ba6d81846");
        let result = hash_concatenated(&[&package_salt_value, &BLOCK_KEYS_DATA_INTEGRITY_HMAC_KEY]);
        assert_eq!(
            result,
            hex!(
    ...
    fn test_convert_password_to_key() {
        let key_salt_value = hex!("3aa973eec73c98c4710021730ef5b513");
        let result =
            convert_password_to_key("password", &key_salt_value, 100_000, 256, &BLOCK_KEYS_KEY);
        assert_eq!(
            result,
