...
/// - The random bytes are generated using the `thread_rng` from the `rand` crate,
///   which provides a fast, thread-local, secure random number generator.
/// - Suitable for generating cryptographic keys, salts, nonces, and other random data.
/// - The macro does not return the byte array; it defines it in the current scope.
/// - Ensure that the generated byte arrays are handled securely and not exposed unintentionally.
#[macro_export]
macro_rules! generate_random_bytes {
    ($var_name:ident, $size:expr) => {
        let mut $var_name = [0u8; $size];
